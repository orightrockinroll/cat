	...
	<script type='text/javascript' src="./js/fullmode.js"></script>
		...
		<script>
			...
			function gotoPage(){
				...
				window.location.href = 'default.php?nav=&pageNumber=' + pageNum + '&emode=0';
			}
		</script>
	 ...
	 $_SESSION['page_no'] = 1;
		...
		//echo $nav . "<br/>";
		   ...
		   if($nav == 'right' && $isEdit == 0){
			...
			if($nav == "" && $editMode == 0 && isset($_GET["pageNumber"])){
			   if($_GET["pageNumber"] == $pageCount_){
					$pageno_ = $_GET["pageNumber"] - 1; 
			   }else{
				   $pageno_ = $_GET["pageNumber"]; 
			   }
			   $_SESSION['page_no'] = $pageno_;
		   }
		   
		   //echo "page number:" . $_POST["pageNumber"];
		   ...
		   if($nav == "last" && $editMode == 0){
			   $_SESSION['page_no'] = $pageCount_ -1;
			   $pageno_ = $_SESSION['page_no']; 
		   }
			
		  //echo "page number: " . $pageno_ . "<br/>";
           ...
           //echo $pageno_ . ' pageCount: ' . $pageCount_;
		   
		   if($pageno_ > $pageCount_){
		    ...
		    $_SESSION['page_no'] = $pageno_ + $pageCount_; //to offset -1 set to 3
			$pageno_ = $_SESSION['page_no'];
		   }
	...
	<div id="divContainer" style="position:absolute;width:1220px;height:800px;margin-left: auto;
       ...
       right: 0;">
		...
		<div style="position:relative;
			...
			<input id="pageNumber" type="number" style="width:60px;" 
				...
				min="1" max="<?php echo $pageCount_ ?>" 
				value="<?php echo $_GET["pageNumber"] ?>" />
			<input id="btnGo" type="button" value="Go" title="Go" onclick="gotoPage()"/>
		</div>
		...
		<div style="float:left;position: relative;width: 600px;" onclick="getXyLocation(event);" >
			...
			<div id="nav_left" style="position:relative; float: left;top:-420px;z-index:2020" 
				ondrop="drop(event, 'left', '<?php echo $isDraggable; ?>')" ondragover="allowDrop(event)">
				<!-- left:-60px; -->
				<a href="default.php?nav=<?php echo 'first&emode='. $isEdit;?>">
				<img id="ImagePrev" src="./images/iv_left_start_arrow.png" align="left" style="border:none;padding-right:5px;" /></a>
			</div>
			<div id="nav_left" style="position:relative; float: left;top:-420px;z-index:2020" 
				ondrop="drop(event, 'left', '<?php echo $isDraggable; ?>')" ondragover="allowDrop(event)"> <!-- left:-60px; -->
	
				<a href="default.php?nav=<?php echo 'left&emode='. $isEdit; $_IsButtonClicked = true;?>">
				<img id="Image1" src="./images/iv_left_next_arrow.png" align="left" style="border:none;" /></a>
			</div> 
		...
		<div style="float:right;position: relative;width: 600px;" onclick="getXyLocation(event);" >
			...
			<div id="nav_right" style="position:relative; float: right;top:-420px;z-index:2020" 
				ondrop="drop(event, 'right', '<?php echo $isDraggable; ?>')" ondragover="allowDrop(event)" >
				<!-- left:60px; -->
				<a href="default.php?nav=<?php echo 'last&emode='. $isEdit;?>">
				<img id="Image2" src="./images/iv_right_end_arrow.png" align="right" style="border:none;" /></a>
			</div>
			<div id="nav_right" style="position:relative; float: right;top:-420px;z-index:2020" 
				ondrop="drop(event, 'left', '<?php echo $isDraggable; ?>')" ondragover="allowDrop(event)"> <!-- left:60px; -->
				<a href="default.php?nav=<?php echo 'right&emode='. $isEdit;?>">
				<img id="Image2" src="./images/iv_right_next_arrow.png" align="right" style="border:none;padding-right:5px;" /></a>
			</div>
		</div>	
		<?php
			//echo $pageLeft;
	   ...
	   <div style="float:right;position: relative;top:-90px;width:1220px;">
		...
		//foreach($catConfigXML as $config) {
			...
			// get the configuration of the total number of pages to be displayed in advance
			$advanceLoadingPages = $catConfigXML['advanceLoadingPages'];
			 ...
			 // start loading the next page of the currently opened page
			 $i = $pageno_ + 1;	 
			 ...
			 if($advLoadingPages <= $pageCount_){
				$advanceLoadingPagesSplit1 = $advLoadingPages;
			 }else{
				$advanceLoadingPagesSplit1 = $pageCount_;
				$advanceLoadingPagesSplit2 = $advLoadingPages - $pageCount_;
			 }
			...
			function displayAdvanceLoadingPage($pageNumber) {
				$img = './pages/pHi'.$pageNumber.'.jpg';
			 ?>
			 <a class="pageGroup" href="<?php echo $img; ?>" >
				<img src="<?php echo $img; ?>" style="width: 60px;height: 70px;border-width:0px; "/>
			 </a>  
			 <?php
			}
		 ?>
